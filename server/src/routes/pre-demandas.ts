...
import { z } from "zod";
import type { PreDemandaStatus } from "../domain/types";
...
export async function registerPreDemandaRoutes(app: FastifyInstance, options: { preDemandaRepository: PreDemandaRepository }) {
  ...
  app.post("/api/pre-demandas", { preHandler: app.authenticate }, async (request, reply) => {
  ...
  app.get("/api/pre-demandas", { preHandler: app.authenticate }, async (request, reply) => {
  ...
  app.get("/api/pre-demandas/:preId", { preHandler: app.authenticate }, async (request, reply) => {
  ...
  app.post("/api/pre-demandas/:preId/associacoes-sei", { preHandler: app.authenticate }, async (request, reply) => {
    const params = z.object({ preId: z.string().trim().min(1) }).parse(request.params);
  ...
  app.get("/api/pre-demandas/:preId/auditoria", { preHandler: app.authenticate }, async (request, reply) => {
    const params = z.object({ preId: z.string().trim().min(1) }).parse(request.params);

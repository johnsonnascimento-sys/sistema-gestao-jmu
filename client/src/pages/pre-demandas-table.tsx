...
export function PreDemandasTable({
  items,
  sectorRiskById,
  onQuickAction,
}: PreDemandasTableProps) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent className="overflow-x-auto">
        {items.length === 0 ? (
          <EmptyState
            description="Ajuste os filtros ou mude para outro preset para encontrar processos nesta fila."
                ...
                >
                  <td className="px-3 py-4 font-semibold text-slate-950">
                    ...
                    {item.metadata.urgente ? (
                      <div className="mt-2 inline-flex rounded-full bg-rose-600 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-white">
                        Urgente
                      </div>
                    ) : null}
                  </td>
                  ...
                  <td className="px-3 py-4">
                    <div className="grid gap-1">
                      <span>
                        {item.setorAtual ? item.setorAtual.sigla : "-"}
                      </span>
                      {item.setorAtual?.id &&
                      sectorRiskById[item.setorAtual.id] !== "normal" ? (
                  ...
                  <td className="px-3 py-4">
                    <div className="flex flex-wrap gap-2">
                      ...
                      {item.metadata.urgente ? (
                        <span className="rounded-full bg-rose-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-rose-700 ring-1 ring-rose-200">
                          Urgente
                        </span>
                      ) : null}
                    </div>
                  ...
                  <td className="px-3 py-4">
                    <div className="grid gap-1">
                      ...
                      {item.status !== "encerrada" ? (
                        <DeadlineStatusPill signal={item.prazoStatus} />
                      ) : null}
                    </div>
                  </td>
                  <td className="px-3 py-4">
                    <div className="grid gap-1">
                      ...
                      {item.status !== "encerrada" ? (
                        <DeadlineStatusPill
                          signal={getDeadlineSignal(item.proximoPrazoTarefa)}
                        />
                      ) : null}
                    </div>
                  </td>

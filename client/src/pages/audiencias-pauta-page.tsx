...
function getSituacaoTone(situacao: string) {
  if (situacao === "designada")
    return "bg-sky-50 text-sky-700 ring-1 ring-sky-200";
  if (situacao === "convertida_diligencia")
...
export function AudienciasPautaPage() {
  ...
  const [appliedFilters, setAppliedFilters] =
    useState<PautaFilters>(EMPTY_FILTERS);
  ...
  async function loadSummary() {
    try {
      ...
      setError(
        formatAppError(nextError, "Falha ao carregar pauta de audiencias."),
      );
    } finally {
      setLoading(false);
    }
  ...
  const filteredAudiencias = useMemo(() => {
    ...
    return [...audiencias]
      .filter((item) => {
        ...
        const searchableProcesso = normalizeText(
          `${item.preNumero} ${item.preId} ${item.numeroJudicial ?? ""}`,
        );
        const searchableMagistrado = normalizeText(item.magistradoNome ?? "");

        if (
          appliedFilters.dataInicio &&
          startDate < appliedFilters.dataInicio
        ) {
          return false;
        }
        ...
        if (
          ...
          item.situacao !== appliedFilters.situacao
        ) {
          return false;
        }
        ...
        if (
          magistradoQuery &&
          !searchableMagistrado.includes(magistradoQuery)
        ) {
          return false;
        }
  ...
  if (loading) {
    return (
      <LoadingState
        description="Carregando pauta de audiencias..."
        ...
        <Card className="overflow-hidden rounded-[24px] border-slate-200 bg-white shadow-sm">
          <CardHeader className="gap-1 border-b border-slate-100 pb-4">
            ...
            <CardDescription className="text-slate-500">
              O formato segue a pauta do e-Proc: processo, magistrado, situacao
              e periodo.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 p-4 md:p-5">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-3">
              <p className="text-sm text-slate-500">
                {filteredAudiencias.length} registro
                {filteredAudiencias.length === 1 ? "" : "s"} encontrado
                {filteredAudiencias.length === 1 ? "" : "s"}.
              </p>
      ...
      >
        <Card className="overflow-hidden rounded-[24px] border-slate-200 bg-white shadow-sm">
          <CardHeader className="border-b border-slate-100 pb-4">
            ...
            <CardDescription className="text-slate-500">
              Lista principal com visualizacao compacta para consulta rapida.
            </CardDescription>
          </CardHeader>
                    ...
                    {filteredAudiencias.map((audiencia) => (
                      ...
                      >
                        <td className="border-b border-slate-100 px-4 py-4">
                          <Link
                            className="w-fit text-base font-semibold text-sky-700 hover:underline"
                            to={buildPreDemandaPath(audiencia.preId)}
                          >
                            {audiencia.preNumero}
                          </Link>
                        </td>
                        <td className="border-b border-slate-100 px-4 py-4">
                          <p className="min-w-[220px] text-sm leading-6 text-slate-700">
                        ...
                        <td className="border-b border-slate-100 px-4 py-4">
                          <span
                            className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] ${getSituacaoTone(audiencia.situacao)}`}
                          >
                            {formatAudienciaSituacao(audiencia.situacao)}
                          </span>
                        </td>
                        <td className="border-b border-slate-100 px-4 py-4">
                          <p className="max-w-[460px] text-sm leading-6 text-slate-700">

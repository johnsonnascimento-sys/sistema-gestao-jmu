...
function getConflictPreId(details: unknown) {
  ...
  const maybePreId =
    (details as Record<string, unknown>).existingPreId ??
    (details as Record<string, unknown>).preId;
  return typeof maybePreId === "string" && maybePreId.length > 0
    ? maybePreId
    : null;
}
...
export function NewPreDemandaPage() {
  ...
  function removeInteressado(pessoaId: string) {
    setSelectedInteressados((current) =>
      current.filter((item) => item.id !== pessoaId),
    );
  }
          ...
          <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
            <div className="md:col-span-2 grid gap-3 rounded-[28px] border border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(240,246,249,0.88))] px-5 py-4 shadow-[0_14px_32px_rgba(20,33,61,0.05)]">
              <FormField label="Pessoas interessadas">
                <Input
                  ...
                  placeholder="Buscar pessoas para vincular como interessadas"
              ...
              {selectedInteressados.length > 0 ? (
                ...
                <p className="text-sm text-slate-500">
                  Nenhuma pessoa interessada selecionada.
                </p>
              )}
            ...
            <div className="md:col-span-2 grid gap-3 rounded-[28px] border border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(240,246,249,0.88))] px-5 py-4 shadow-[0_14px_32px_rgba(20,33,61,0.05)]">
              <div>
                ...
                <p className="text-xs font-medium uppercase tracking-[0.2em] text-slate-500">
                  Assuntos com procedimentos criam tarefas automáticas ao
                  salvar.
                </p>
              </div>
                      ...
                      onChange={(event) =>
                        setSelectedAssuntoIds((current) =>
                          event.target.checked
                            ? [...current, assunto.id]
                            : current.filter((item) => item !== assunto.id),
                        )
                      }
                      type="checkbox"
                    />
              ...
              {selectedAssuntoIds.length ? (
                <div className="rounded-2xl border border-sky-100 bg-sky-50 px-4 py-3 text-sm text-sky-900">
                  ...
                  <p className="mt-1 text-sky-800">
                    {selectedAssuntoIds.length} assunto
                    {selectedAssuntoIds.length === 1 ? "" : "s"} selecionado
                    {selectedAssuntoIds.length === 1 ? "" : "s"}.
                    {assuntos.some(
                      (assunto) =>
                        selectedAssuntoIds.includes(assunto.id) &&
                        assunto.procedimentos.length > 0,
                    )
            ...
            <div className="md:col-span-2 rounded-[28px] border border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(247,241,233,0.82))] px-5 py-4 shadow-[0_14px_32px_rgba(20,33,61,0.05)]">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs font-medium uppercase tracking-[0.2em] text-slate-500">
                    marcadores operacionais do processo
                  </p>
                </div>
                <Button
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  <label className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3">
                    <span>
                      ...
                      <span className="block text-xs text-slate-500">
                        Marca o processo em filtros e dashboard.
                      </span>
                    </span>
                    <input
                  ...
                  <label className="flex items-center justify-between rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3">
                    <span>
                      ...
                      <span className="block text-xs text-slate-500">
                        Destaca o processo no dashboard e nas filas.
                      </span>
                    </span>
                    <input
                  ...
                  <p className="md:col-span-2 text-xs font-medium uppercase tracking-[0.18em] text-slate-500">
                    Tarefas recorrentes sao configuradas no detalhe do processo,
                    cada uma com prazo proprio de conclusao.
                  </p>
                </div>
              ) : null}
            </div>
            ...
            {error ? (
              <div className="md:col-span-2 rounded-3xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700">
                ...
                {conflictPreId ? (
                  <Link
                    className="underline"
                    to={buildPreDemandaPath(conflictPreId)}
                  >
            ...
            {result ? (
              <div className="md:col-span-2 rounded-3xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700">
                {result.idempotent
                  ? "Processo existente localizado."
                  : "Processo criado com sucesso."}{" "}
                <Link
                  className="underline"
                  to={buildPreDemandaPath(result.preId)}
                >
                  {result.preId}
                </Link>
              </div>

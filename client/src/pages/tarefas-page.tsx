...
function formatTaskTime(
  task: Pick<DashboardTaskItem, "horarioInicio" | "horarioFim">,
) {
    ...
    >
      <div className="grid gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span
            className={`rounded-full px-3 py-1 text-[11px] font-bold uppercase tracking-[0.16em] ${getTaskDeadlineTone(task.prazoConclusao)}`}
          >
        ...
        <div className="flex flex-wrap gap-3 text-xs text-slate-500">
          ...
          {task.setorDestinoSigla ? (
            <span>Setor destino: {task.setorDestinoSigla}</span>
          ) : null}
          <span>
            Criada em {new Date(task.createdAt).toLocaleString("pt-BR")}
          </span>
          {task.concluida && task.concluidaEm ? (
            <span>
              Concluida em {new Date(task.concluidaEm).toLocaleString("pt-BR")}
            </span>
          ) : null}
        </div>
      </div>
  ...
  return (
    <Card className="rounded-[28px] border border-amber-200/80 bg-[linear-gradient(180deg,rgba(255,251,235,0.95),rgba(255,247,237,0.88))] shadow-[0_12px_24px_rgba(120,53,15,0.05)]">
      <CardHeader>
        ...
        <CardDescription>
          {total} processo(s) aberto(s) sem tarefa cadastrada.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {items.length === 0 ? (
          <EmptyState
            title="Sem lacunas operacionais"
            description="Todos os processos abertos possuem ao menos uma tarefa cadastrada."
          />
        ) : (
          <div className="grid gap-3">
            {items.map((item) => (
  ...
  return (
    <Card className="rounded-[28px] border border-amber-200/80 bg-[linear-gradient(180deg,rgba(255,251,235,0.95),rgba(255,247,237,0.88))] shadow-[0_12px_24px_rgba(120,53,15,0.05)]">
      <CardHeader>
        ...
        <CardDescription>
          Audiências designadas e suas pendências, inclusive processos sem tarefa pendente.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {items.length === 0 ? (
          <EmptyState
            title="Sem processos com audiência"
              ...
              >
                ...
                <div className="mt-3 grid gap-2 border-t border-amber-100 pt-3">
                  <p className="text-xs font-semibold uppercase tracking-[0.14em] text-amber-800">
                    {item.tarefasPendentes.length === 0
                      ? "Sem tarefas pendentes"
                      : `${item.tarefasPendentes.length} tarefa(s) pendente(s)`}
                  </p>
                  {item.tarefasPendentes.map((task) => {
                    ...
                    return (
                      <div
                        key={task.id}
                        className="rounded-xl border border-amber-100 bg-amber-50/60 px-3 py-2"
                      >
                        <div className="flex flex-wrap items-center gap-2">
                          <p className="text-sm font-semibold text-slate-900">
                            {task.descricao}
                          ...
                          {task.urgente ? (
                            <span className="rounded-full bg-rose-600 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.14em] text-white">
                              Urgente
                            </span>
                          ) : null}
                          <span
                            className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.14em] ${getTaskDeadlineTone(task.prazoConclusao)}`}
                          >
                            Prazo {formatDateOnlyPtBr(task.prazoConclusao)}
                          </span>
                        </div>
...
export function TarefasPage() {
  ...
  if (loading) {
    return (
      <LoadingState
        description="Carregando tarefas pendentes e concluidas."
        title="Tarefas"
      />
    );
  }
    ...
    <section className="grid gap-6">
        ...
        <Card className="rounded-[28px] border border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.95),rgba(240,246,249,0.88))] shadow-[0_12px_24px_rgba(20,33,61,0.05)]">
          <CardHeader className="gap-4">
            <div>
              ...
              <CardDescription>
                {counts.pendentes} pendente(s) e {counts.concluidas}{" "}
                concluida(s).
              </CardDescription>
            </div>
            <div className="grid gap-2 sm:max-w-sm">
              ...
              <select
                ...
                id="task-sort-mode"
                onChange={(event) =>
                  setSortMode(event.target.value as DashboardTaskSortMode)
                }
                value={sortMode}
              >
            ...
            <div className="grid gap-2 sm:max-w-xs">
              <label
                className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500"
                htmlFor="task-recurrence-filter"
              >
                Recorrencia
              </label>
              <select
          ...
          <CardContent>
            <Tabs
              onValueChange={(value) =>
                setCurrentTab(value as DashboardTaskStatusFilter)
              }
              value={currentTab}
            >
      ...
      >
        <div className="mb-3 grid gap-2 sm:max-w-md">
          <label
            className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500"
            htmlFor="open-without-tasks-filter"
          >
        ...
        <OpenProcessesWithoutTasksCard
          items={openProcessesWithoutTasks.items}
          total={openProcessesWithoutTasks.total}
        />
      </motion.div>
    </section>
  );

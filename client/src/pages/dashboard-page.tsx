...
function getTaskDeadlineState(prazoConclusao: string) {
  ...
  return {
    ...
    containerClass:
      "border-white/80 bg-gradient-to-br from-white/95 to-slate-50/80 shadow-md",
...
export function DashboardPage() {
  const [summary, setSummary] = useState<PreDemandaDashboardSummary | null>(
    null,
  );
  ...
  function renderQueueItem(
    ...
    if (highlightType === "urgent") {
      borderStyle =
        "border-rose-200/80 bg-gradient-to-br from-rose-50/95 to-white/80 backdrop-blur-lg shadow-rose-100/50 shadow-md hover:shadow-xl hover:-translate-y-1";
      titleColor = "text-rose-600";
    } else if (highlightType === "payment") {
      borderStyle =
        "border-amber-200/80 bg-gradient-to-br from-amber-50/95 to-white/80 backdrop-blur-lg shadow-amber-100/50 shadow-md hover:shadow-xl hover:-translate-y-1";
    ...
    return (
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            <p
              className={`text-xs font-bold uppercase tracking-[0.24em] ${titleColor}`}
            >
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            {item.metadata.pagamentoEnvolvido ? (
              <span className="rounded-full bg-amber-600 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.18em] text-white">
                Pagamento
              </span>
            ) : null}
        ...
        <div className="grid gap-1 text-sm text-slate-500">
          ...
          {highlightType !== "urgent" && highlightType !== "payment" && (
            <p>Pessoa: {item.interessados.length}</p>
          )}
  ...
  return (
    <div className="grid gap-6">
          ...
          actions={
            <div className="flex gap-2">
              <Button asChild variant="secondary">
                <Link to="/pre-demandas?preset=fila-operacional">
                  Fila Operacional
                </Link>
              </Button>
              <Button asChild>
      ...
      >
        ...
        >
          <Link to="/pre-demandas?preset=aguardando-sei">Aguardando SEI</Link>
        </Button>
        <Button
          asChild
          variant="outline"
        ...
        <Button
          ...
          className="h-8 rounded-full bg-white"
        >
          <Link to="/pre-demandas?preset=em-risco">Em risco</Link>
        </Button>
        <Button
          asChild
          variant="outline"
          size="sm"
          className="h-8 rounded-full bg-white text-rose-600 border-rose-200"
        >
        ...
        <Button
          ...
          className="h-8 rounded-full bg-white"
        >
          <Link to="/pre-demandas?preset=ultimas-encerradas">
            Últimos Encerrados
          </Link>
        </Button>
        <Button asChild variant="ghost" size="sm" className="h-8">
          <Link to="/pre-demandas">Acessar Busca Avançada &rarr;</Link>
        </Button>
      </motion.div>
      ...
      >
        {summary.counts.map((item) => (
          <MetricCard
            ...
            to={statusMetricHref[item.status]}
            value={item.total}
          />
        ))}
        <MetricCard
          label="Paradas 2d+"
          to={buildAnalyticalTableHref({
            queueHealth: "attention,critical",
            sortBy: "updatedAt",
            sortOrder: "asc",
          })}
          value={summary.agingAttentionTotal + summary.agingCriticalTotal}
        />
        <MetricCard
          label="Em risco 5d+"
          to={buildAnalyticalTableHref({ preset: "em-risco" })}
          value={summary.agingCriticalTotal}
        />
        <MetricCard
          label="Vence hoje"
          to={buildAnalyticalTableHref({ preset: "vence-hoje" })}
          value={summary.dueTodayTotal}
        />
        <MetricCard
          label="Prazos na semana"
          to={buildAnalyticalTableHref({ preset: "vencem-na-semana" })}
          value={summary.deadlines.processo.dueSoonTotal}
        />
        <MetricCard
          label="Sem setor"
          to={buildAnalyticalTableHref({ preset: "sem-setor" })}
          value={summary.withoutSetorTotal}
        />
        <MetricCard
          label="Sem envolvidos"
          to={buildAnalyticalTableHref({ preset: "sem-envolvidos" })}
        ...
        <Card className="border-white/60 bg-white/50 backdrop-blur-xl shadow-xl rounded-[32px] overflow-hidden">
          <CardHeader className="pb-4">
            ...
            <CardDescription className="text-slate-500">
              Prazos do processo, tarefas e consumo do tempo geral.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 xl:grid-cols-3">
            {[
              ...
              {
                id: "tarefas",
                label: "Processos com tarefa pendente",
                campo: "proximoPrazoTarefa",
                secondary: "processesWithPendingTasks",
              },
              {
                id: "prazo",
              ...
              if (item.id === "prazo") {
                ...
                const noPrazo = Math.max(
                  (summary.deadlines.processo?.totalDefined ?? 0) - atrasados,
                  0,
                );
                const data = [
                ...
                return (
                  <article
                    className="group relative grid grid-cols-[1fr_100px] gap-3 items-center overflow-hidden rounded-[24px] border border-white/80 bg-gradient-to-br from-white/90 to-slate-50/80 px-5 py-5 shadow-lg backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
                    key={item.id}
                  >
              ...
              const metrics =
                summary.deadlines[item.id as "processo" | "tarefas"];
              const data = [
                {
                  name: "Vencidos",
                ...
                {
                  name: "Vence hoje",
                  value: metrics.dueTodayTotal,
                  color: "#f59e0b",
                },
                {
                  name: "Na semana",
                  value: metrics.dueSoonTotal,
                  color: "#0ea5e9",
                },
              ].filter((d) => d.value > 0);
              ...
              const total =
                ...
                metrics.dueTodayTotal +
                metrics.dueSoonTotal;
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col h-full justify-between">
                    <div>
                      ...
                      <h3 className="mt-2 text-3xl font-light tracking-tight text-slate-900">
                        {"processesWithPendingTasks" in metrics
                          ? metrics.processesWithPendingTasks
                          : metrics.totalDefined}
                      </h3>
                      <p className="text-xs font-medium text-slate-400">
                        {"processesWithPendingTasks" in metrics
                          ? "processos com tarefas abertas"
                          : "processos com prazo"}
                      </p>
                    </div>
                    <div className="mt-4 grid gap-2 text-xs font-medium text-slate-600">
                      <Link
                        className="flex w-fit items-center gap-2 hover:text-rose-600 transition-colors"
                        to={buildAnalyticalTableHref({
                          deadlineCampo: item.campo,
                          prazoRecorte: "overdue",
                        })}
                      >
                        <div className="w-2 h-2 rounded-full bg-rose-500"></div>
                      ...
                      <Link
                        className="flex w-fit items-center gap-2 hover:text-amber-600 transition-colors"
                        to={buildAnalyticalTableHref({
                          deadlineCampo: item.campo,
                          prazoRecorte: "today",
                        })}
                      ...
                      <Link
                        className="flex w-fit items-center gap-2 hover:text-sky-600 transition-colors"
                        to={buildAnalyticalTableHref({
                          deadlineCampo: item.campo,
                          prazoRecorte: "soon",
                        })}
        ...
        <Card className="overflow-hidden rounded-[32px] border-amber-200/70 bg-gradient-to-br from-amber-50/95 via-white/90 to-amber-100/60 shadow-xl">
          <CardHeader className="gap-3 pb-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="grid gap-1">
              <CardTitle className="text-xl font-light tracking-tight text-amber-950">
                Pauta de audiências
              </CardTitle>
              <CardDescription className="text-amber-800/80">
                Processos com audiência permanecem aqui até o processo ser
                concluído, independentemente do status do ato.
              </CardDescription>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex h-8 items-center rounded-full bg-amber-100 px-3 text-[11px] font-bold uppercase tracking-[0.18em] text-amber-800 ring-1 ring-amber-200">
                {upcomingAudiencias.length}{" "}
                {upcomingAudiencias.length === 1 ? "audiência" : "audiências"}
              </span>
              <Button
                asChild
                size="sm"
                variant="outline"
                className="h-8 rounded-full border-amber-200 bg-white shadow-sm"
              >
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      <div className="min-w-0 flex-1">
                        ...
                        <h3 className="mt-2 break-words text-base font-semibold leading-snug text-slate-950">
                          {audiencia.assunto}
                        </h3>
                        <p className="mt-2 text-sm text-slate-600">
                          {formatDateTimePtBr(audiencia.dataHoraInicio)}
                        </p>
                        {audiencia.magistradoNome ? (
                          <p className="mt-1 text-sm text-slate-500">
                            Magistrado: {audiencia.magistradoNome}
                          </p>
                        ) : null}
                      </div>
                    ...
                    {(audiencia.observacoes ?? audiencia.descricao) ? (
                    ...
                    {audiencia.dataHoraFim ? (
                      <p className="text-sm font-medium text-slate-700">
                        Término previsto:{" "}
                        {formatDateTimePtBr(audiencia.dataHoraFim)}
                      </p>
                    ) : null}
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-slate-400">
                        Abrir processo para detalhes da audiência
                      </p>
                      <Button
                        asChild
                        size="sm"
                        variant="outline"
                        className="h-9 rounded-full border-amber-200 bg-white shadow-sm"
                      >
                        <Link to={buildPreDemandaPath(audiencia.preId)}>
                          Ver processo
                        </Link>
                      </Button>
                    </div>
      ...
      >
        <Card className="h-fit rounded-[32px] overflow-hidden border-white/60 bg-white/50 backdrop-blur-xl shadow-xl">
          <CardHeader className="pb-4">
            <CardTitle className="text-xl font-light tracking-tight text-slate-800">
              Filas de Ação Imediata
            </CardTitle>
            <CardDescription className="text-slate-500">
              Filas de trabalho prioritárias para desafogar a operação.
            </CardDescription>
          </CardHeader>
            ...
            <Tabs defaultValue="urgentes" className="w-full">
              <TabsList className="flex flex-wrap h-auto gap-2 p-4 justify-start bg-transparent">
                ...
                >
                  <span>Pagamento</span>
                  <span className="flex h-5 items-center justify-center rounded-full bg-slate-100 px-2 text-[10px] font-bold text-slate-600 group-data-[state=active]:bg-white/20 group-data-[state=active]:text-white">
                    {summary.paymentMarkedItems.length}
                  </span>
                </TabsTrigger>
                <TabsTrigger
                ...
                >
                  ...
                  <span className="flex h-5 items-center justify-center rounded-full bg-slate-100 px-2 text-[10px] font-bold text-slate-600 group-data-[state=active]:bg-white/20 group-data-[state=active]:text-white">
                    {summary.awaitingSeiItems.length}
                  </span>
                </TabsTrigger>
                <TabsTrigger
                  value="parados"
                ...
                >
                  ...
                  <span className="flex h-5 items-center justify-center rounded-full bg-slate-100 px-2 text-[10px] font-bold text-slate-600 group-data-[state=active]:bg-white/20 group-data-[state=active]:text-white">
                    {summary.dueSoonItems.length +
                      summary.withoutSetorItems.length +
                      summary.withoutInteressadosItems.length}
                  </span>
                </TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="pagamento" className="grid gap-3">
                {summary.paymentMarkedItems.length === 0 ? (
                  <div className="py-8">
                    <EmptyState
                      description="Nenhum processo assinalado com impacto de pagamento."
                      title="Limpo"
                    />
                  </div>
                ) : (
                  summary.paymentMarkedItems.map((item) =>
                    renderQueueItem(item, "payment"),
                  )
                )}
              </TabsContent>
              ...
              <TabsContent value="aguardando_sei" className="grid gap-3">
                {summary.awaitingSeiItems.length === 0 ? (
                  <div className="py-8">
                    <EmptyState
                      description="Nenhuma demanda aguardando abertura de processo SEI para seguir."
                      title="Fila Limpa"
                    />
                  </div>
                ) : (
              ...
              <TabsContent value="parados" className="grid gap-3">
                {staleItems.length === 0 ? (
                  <div className="py-8">
                    <EmptyState
                      description="Tudo fluindo regularmente. Fila pedindo andamento está zerada."
              ...
              <TabsContent value="pendencias" className="grid gap-6">
                <div className="grid gap-3">
                  <div className="flex items-center justify-between gap-3 text-slate-800">
                    <p className="text-sm font-semibold">
                      Prazos na semana ({summary.dueSoonItems.length})
                    </p>
                    <Link
                      className="text-sm font-medium hover:underline text-blue-600"
                      to="/pre-demandas?preset=vencem-na-semana"
                    >
                      Ver todos
                    </Link>
                  </div>
                  {summary.dueSoonItems.length === 0 ? (
                    <p className="rounded-[20px] border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-500">
                      Nenhum prazo iminente registrado.
                    </p>
                  ) : (
                    summary.dueSoonItems
                      .slice(0, 3)
                ...
                <div className="grid gap-3">
                  <div className="flex items-center justify-between gap-3 text-slate-800">
                    <p className="text-sm font-semibold">
                      Sem setor atual ({summary.withoutSetorItems.length})
                    </p>
                    <Link
                      className="text-sm font-medium hover:underline text-blue-600"
                      to="/pre-demandas?preset=sem-setor"
                    >
                      Ver todos
                    </Link>
                  </div>
                  {summary.withoutSetorItems.length === 0 ? (
                    <p className="rounded-[20px] border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-500">
                      Todos os processos ativos já possuem setor.
                    </p>
                  ) : (
                    summary.withoutSetorItems
                      .slice(0, 3)
                      .map((item) => renderQueueItem(item))
                  )}
                </div>
                ...
                <div className="grid gap-3">
                  <div className="flex items-center justify-between gap-3 text-slate-800">
                    <p className="text-sm font-semibold">
                      Sem envolvidos ({summary.withoutInteressadosItems.length})
                    </p>
                    <Link
                      className="text-sm font-medium hover:underline text-blue-600"
                      to="/pre-demandas?preset=sem-envolvidos"
                    >
                      Ver todos
                    </Link>
                  </div>
                  {summary.withoutInteressadosItems.length === 0 ? (
                    <p className="rounded-[20px] border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-500">
                      Os processos ativos já possuem envolvidos vinculados.
                    </p>
                  ) : (
                    summary.withoutInteressadosItems
                      .slice(0, 3)
                      .map((item) => renderQueueItem(item))
                  )}
                </div>
        ...
        <Card className="flex h-[800px] flex-col rounded-[32px] overflow-hidden border-white/60 bg-white/50 backdrop-blur-xl shadow-xl">
          <CardHeader className="shrink-0 gap-3 pb-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="grid gap-1">
              <CardTitle className="text-xl font-light tracking-tight text-slate-800">
                Tarefas abertas mais antigas
              </CardTitle>
              <CardDescription className="text-slate-500">
                Ordenadas pelo prazo de conclusão mais antigo.
              </CardDescription>
            </div>
            <Button
              asChild
              variant="outline"
              size="sm"
              className="h-9 rounded-full border-slate-200 bg-white shadow-sm"
            >
          ...
          <CardContent className="grid gap-3 overflow-y-auto pr-2 pb-6">
            {summary.oldestOpenTasks.length === 0 ? (
              <EmptyState
                description="Nenhuma tarefa aberta para exibir."
                title="Fila limpa"
              />
            ) : (
              summary.oldestOpenTasks.map((task) => {
                ...
                return (
                  <Link
                    ...
                    key={task.id}
                    to={buildPreDemandaPath(task.preId)}
                  >
                    <div
                    ...
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      <div className="min-w-0 flex-1">
                        ...
                        <h3
                          className={`mt-2 break-words text-base font-semibold leading-snug ${deadlineState.titleClass}`}
                        >
                          {task.descricao}
                        </h3>
                      </div>
                      <div className="grid shrink-0 gap-2 justify-items-start md:justify-items-end">
                        ...
                        <span
                          className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] ${deadlineState.dateClass}`}
                        >
                    ...
                    <div
                      className={`grid gap-1 text-sm ${deadlineState.subtitleClass}`}
                    >
                      <p>{task.assunto}</p>
                      <p>
                        {task.setorDestinoSigla
                          ? `Setor destino: ${task.setorDestinoSigla}`
                          : "Sem setor destino"}
                      </p>

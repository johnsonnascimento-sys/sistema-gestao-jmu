...
// Fixes widget positioning for Busca_Normas page (use grid fields topRow/bottomRow/leftColumn/rightColumn).
...
async function main() {
  ...
  const targets = {
    Input_Busca: { leftColumn: 0, rightColumn: 40, topRow: 0, bottomRow: 6 },
    Btn_Buscar: { leftColumn: 41, rightColumn: 56, topRow: 0, bottomRow: 6 },
    Input_ApiKey: { leftColumn: 0, rightColumn: 40, topRow: 7, bottomRow: 13 },
    Btn_SalvarKey: { leftColumn: 41, rightColumn: 56, topRow: 7, bottomRow: 13 },
    Table_Resultados: { leftColumn: 0, rightColumn: 56, topRow: 14, bottomRow: 46 },
  };

  let touched = 0;
  for (const w of dsl.children) {
    const t = targets[w.widgetName];
    if (!t) continue;
    Object.assign(w, t);
    if (typeof w.detachFromLayout !== "boolean") w.detachFromLayout = false;
    if (typeof w.zIndex !== "number") w.zIndex = 1;
    // Clear the non-grid positional fields that can cause weird overlays.
    delete w.left;
    delete w.right;
    delete w.top;
    delete w.bottom;

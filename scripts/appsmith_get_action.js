...
// Debug helper: fetch and print a minimal view of an Appsmith action by id.
// Usage: node scripts/appsmith_get_action.js <actionId>
//
...
async function main() {
  ...
  if (!baseUrl || !email || !password) {
    throw new Error("Missing APPSMITH_URL/APPSMITH_EMAIL/APPSMITH_PASSWORD (check MEUS_SEGREDOS.txt)");
  }

  const actionId = process.argv[2];
  if (!actionId) throw new Error("Usage: node scripts/appsmith_get_action.js <actionId>");
  ...
  const r = await request("get", `/api/v1/actions/${encodeURIComponent(actionId)}`);
  ...
  const minimal = data
    ? {
        ...
        datasourceId: data.datasource?.id || null,
        pageId: data.pageId || null,
        collectionId: data.collectionId || null,
        isValid: data.isValid,
        updatedAt: data.updatedAt,
        // actionConfiguration differs by plugin; keep a tiny, safe subset.
        actionConfigurationKeys: data.actionConfiguration ? Object.keys(data.actionConfiguration) : [],
      }
    : null;

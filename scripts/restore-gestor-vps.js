...
function buildRemoteScript(options) {
  ...
  return [
    ...
    `BACKUP_SCHEMA=${bashSingleQuote(options.schemaName)}`,
    ...
    'docker run --rm --network "$DOCKER_NETWORK" -e DATABASE_URL="$DATABASE_URL" -e SCHEMA_NAME="$SCHEMA_NAME" -e PRE_RESTORE_SQL_BASENAME="$PRE_RESTORE_SQL_BASENAME" -v "$BACKUP_DIR:/backup" "$PG_IMAGE" /bin/sh -lc \'pg_dump --no-owner --no-privileges --schema "$SCHEMA_NAME" -f "/backup/$PRE_RESTORE_SQL_BASENAME" "$DATABASE_URL"\'',
    ...
    'docker run --rm --network "$DOCKER_NETWORK" -e DATABASE_URL="$DATABASE_URL" -e SCHEMA_NAME="$SCHEMA_NAME" "$PG_IMAGE" /bin/sh -lc \'printf "drop schema if exists %s cascade;\\n" "$SCHEMA_NAME" | psql "$DATABASE_URL" -v ON_ERROR_STOP=1\'',
    'docker run --rm --network "$DOCKER_NETWORK" -e DATABASE_URL="$DATABASE_URL" -v "$BACKUP_DIR:/backup" "$PG_IMAGE" /bin/sh -lc \'gunzip -c "$1" | psql "$DATABASE_URL" -v ON_ERROR_STOP=1\' -- "$TARGET_FILE"',
    "",

...
// Attempts to change an action runBehaviour and verifies the result by re-fetching actions.
//
// Usage:
//   node scripts/appsmith_set_action_runbehaviour.js GerarEmbedding MANUAL
//
...
async function main() {
  ...
  const actionName = process.argv[2] || "GerarEmbedding";
  const runBehaviour = (process.argv[3] || "MANUAL").toUpperCase();
  ...
  console.log(JSON.stringify({ actionName, id: a.id, before, requested: runBehaviour, after }, null, 2));
}

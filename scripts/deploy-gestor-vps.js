...
function buildRemoteScript(options) {
  ...
  return [
    ...
    'mkdir -p "$BACKUP_DIR"',
    'docker rm -f "$CONTAINER_NAME" >/dev/null 2>&1 || true',
    'docker run -d --name "$CONTAINER_NAME" --restart unless-stopped --env-file .env -e OPS_BACKUP_DIR=/backup/ops -e OPS_BACKUP_SCHEMA="$BACKUP_SCHEMA" -v "$BACKUP_DIR:/backup/ops:ro" -p "$PORT_BIND" "$CONTAINER_NAME:latest" >/dev/null',
    "",
    "sleep 15",

...
function buildRemoteScript(options) {
  ...
  return [
    ...
    'NEW_COMMIT_AT="$(git show -s --format=%cI "$NEW_COMMIT")"',
    'VITE_ESCALA_PLANTAO_URL="$(grep -E \'^VITE_ESCALA_PLANTAO_URL=\' .env | tail -n 1 | cut -d= -f2- || true)"',
    'COMMIT_IMAGE="$CONTAINER_NAME:commit-$NEW_COMMIT"',
    'echo "new_commit=${NEW_COMMIT}"',
    'echo "new_commit_at=${NEW_COMMIT_AT}"',
    'echo "vite_escala_plantao_url=${VITE_ESCALA_PLANTAO_URL}"',
    ...
    'docker build --build-arg APP_COMMIT_SHA="$NEW_COMMIT" --build-arg APP_COMMIT_AT="$NEW_COMMIT_AT" --build-arg VITE_ESCALA_PLANTAO_URL="$VITE_ESCALA_PLANTAO_URL" -t "$CONTAINER_NAME:latest" -t "$COMMIT_IMAGE" .',
